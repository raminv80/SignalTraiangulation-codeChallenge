...
Class Circle {
    ...
    public function isEqual(Circle $otherCircle) {
        return bccomp($this->radius, $otherCircle->radius, PRECISION) === 0 &&
               bccomp($this->x, $otherCircle->x, PRECISION) === 0 &&
               bccomp($this->y, $otherCircle->y, PRECISION) === 0;
    }
    ...
    public function calcIntersections(Circle $otherCircle) {
        if ($this->doIntersect($otherCircle)) {
            ...
            $val1   = ( $this->x + $otherCircle->x ) / 2 +
                      ( $otherCircle->x - $this->x ) *
                      ( $thisR2 - $thatR2 ) / ( 2 * $d2 );
            $val2   = 2 * ( $this->y - $otherCircle->y ) * $area / $d2;
            $x1     = $val1 + $val2;
            $x2     = $val1 - $val2;

            // Calculating y axis intersection values
            //---------------------------------------
            $val1 = ( $this->y + $otherCircle->y ) / 2 +
                    ( $otherCircle->y - $this->y ) *
                    ( $thisR2 - $thatR2 ) / ( 2 * $d2 );
            $val2 = 2 * ( $this->x - $otherCircle->x ) * $area / $d2;
            $y1   = $val1 - $val2;
            $y2   = $val1 + $val2;
            ...
            $radius = pow( $x1 - $this->x, 2 ) + pow( $y1 - $this->y, 2 );
